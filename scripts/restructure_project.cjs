...
async function main(){
  ...
  if(await exists(backendPath)){
    if(await exists(serverPath)){
      ...
      // try rename; if it fails (permissions or file locks), move per-item
      try {
        await fsp.rename(backendPath, serverPath);
        console.log('[renamed] backend -> server');
      } catch(renameErr){
        console.log(`[rename-failed] ${renameErr.message}. Falling back to per-item move.`);
        ...
        try{ await fsp.rmdir(backendPath); }catch(e){}
        console.log('[moved-contents] backend -> server');
      }
    }
  } else {
