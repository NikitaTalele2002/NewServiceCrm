import { sequelize } from './db.js';
import { QueryTypes } from 'sequelize';

try {
  console.log('\n' + '='.repeat(80));
  console.log('📋 CHECKING CURRENT STATUS AND SUBSTATUS DATA');
  console.log('='.repeat(80));

  // Get all statuses
  console.log('\n🔍 STATUSES IN DATABASE:');
  const statuses = await sequelize.query(`
    SELECT status_id, status_name
    FROM [status]
    ORDER BY status_id
  `, { type: QueryTypes.SELECT });

  console.log('\nStatus Table:');
  statuses.forEach(s => {
    console.log(`  ${s.status_id}: "${s.status_name}"`);
  });

  // Get all substatus
  console.log('\n🔍 SUB-STATUSES IN DATABASE:');
  const substatus = await sequelize.query(`
    SELECT status_id, sub_status_id, sub_status_name
    FROM [sub_status]
    ORDER BY status_id, sub_status_id
  `, { type: QueryTypes.SELECT });

  console.log('\nSub-Status Table:');
  substatus.forEach(s => {
    console.log(`  Status ${s.status_id} -> Sub-Status ${s.sub_status_id}: "${s.sub_status_name}"`);
  });

  // Check which are being used in calls table
  console.log('\n🔍 ACTUAL STATUS VALUES USED IN CALLS (Sample):');
  const sampleCalls = await sequelize.query(`
    SELECT TOP 5 call_id, call_status_id, call_sub_status_id
    FROM calls
  `, { type: QueryTypes.SELECT });

  console.log('\nSample Calls:');
  sampleCalls.forEach(c => {
    console.log(`  Call ${c.call_id}: status_id=${c.call_status_id}, sub_status_id=${c.call_sub_status_id}`);
  });

  process.exit(0);
} catch (e) {
  console.error('❌ Error:', e.message);
  console.error('Stack:', e.stack);
  process.exit(1);
}
