/**
 * Create final 9 tables with ALL FKs fixed to avoid circular cascade paths
 */
...
async function main() {
  try {
    ...
    console.log('Creating final 9 tables with corrected all FK constraints\n');
    ...
    const sqlStatements = [
      ...
      },
      {
        ...
        sql: `CREATE TABLE [call_technician_assignment] (
          ...
          CONSTRAINT [FK_cta_tech] FOREIGN KEY ([technician_id]) REFERENCES [technicians] ([technician_id]) ON DELETE NO ACTION,
    ...
    for (const tbl of sqlStatements) {
      try {
        ...
        if (err.parent && err.parent.errors && err.parent.errors[0]) {
          console.log(`   ${err.parent.errors[0].message.substring(0, 80)}`);
        }
      }
    }
    ...
    if (verify[0].cnt === 54) {
      console.log('🎉 ALL 54 MODELS SUCCESSFULLY SYNCED!');
    }
