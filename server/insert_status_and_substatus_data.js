/**
 * Insert status and sub_status data for complaint management system
 */
...
async function insertStatusAndSubStatusData() {
  try {
    console.log('🔄 Connecting to database...');
    await sequelize.authenticate();
    console.log('✅ Database connected\n');

    // Define the status structure with sub-statuses
    const statusData = [
      {
        status_name: 'open',
        sub_statuses: []
      ...
      {
        status_name: 'assigned to the technician',
        sub_statuses: []
      },
      {
        status_name: 'pending',
    ...
    console.log('📋 Status structure to insert:');
    ...
    // Insert statuses and sub-statuses
    for (const statusItem of statusData) {
      try {
        ...
        if (existingStatus.length > 0) {
          ...
          console.log(`\n✅ Status "${statusItem.status_name}" already exists (ID: ${statusId})`);
        } else {
          // Insert new status
