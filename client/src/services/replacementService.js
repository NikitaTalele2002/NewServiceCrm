...
export const getProductGroupsApi = async () => {
  ...
  const response = await fetch(getApiUrl('/products/groups'), {
...
export const getProductsByGroupApi = async (groupId) => {
  const token = localStorage.getItem('token');
  const response = await fetch(getApiUrl(`/products/products/${groupId}`), {
    headers: { Authorization: `Bearer ${token}` }
  });
...
export const getModelsByProductApi = async (productId) => {
  const token = localStorage.getItem('token');
  const response = await fetch(getApiUrl(`/products/models/${productId}`), {
    headers: { Authorization: `Bearer ${token}` }
  });
...
export const getServiceCentersApi = async () => {
  const token = localStorage.getItem('token');
  const response = await fetch(getApiUrl('/servicecenter/all'), {
    headers: { Authorization: `Bearer ${token}` }
  });
...
export const createReplacementRequestApi = async (replacementData) => {
  const token = localStorage.getItem('token');
  const response = await fetch(getApiUrl('/spare-requests/replacement'), {
    method: 'POST',
    headers: {
...
export const getReplacementHistoryApi = async (filters = {}) => {
  ...
  const response = await fetch(getApiUrl(`/spare-requests/replacement-history?${params.toString()}`), {
    headers: { Authorization: `Bearer ${token}` }
  });
