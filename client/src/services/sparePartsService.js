...
export const getSparePartsByProductApi = async (productId, modelId) => {
  ...
  let url = `/api/products/spares/product/${productId}`;
  if (modelId) {
    url += `?modelId=${modelId}`;
  }
...
export const createSpareReturnRequestApi = async (returnData) => {
  const token = localStorage.getItem('token');
  const response = await fetch('/api/spare-parts/return-request', {
    method: 'POST',
    headers: {
...
export const getSpareInventoryApi = async (centerId) => {
  ...
  const response = await fetch(`/api/spare-parts/inventory${params}`, {
    headers: { Authorization: `Bearer ${token}` }
  });
